...
class SubmitChangelistWidget(QDialog):
    ...
    def __init__(self, parent=None, myp4=None, change_item=None, file_dict=None):
        ...
        # Load saved contexts
        self.saved_contexts = self.load_saved_contexts()
        ...
        # Make columns resizable - allow user to manually resize all columns
        header = self.files_table_widget.horizontalHeader()
        header.setSectionResizeMode(QHeaderView.Interactive)

        # Set reasonable default widths
        self.files_table_widget.setColumnWidth(0, 30)  # Checkbox
        self.files_table_widget.setColumnWidth(1, 200)  # File
        self.files_table_widget.setColumnWidth(2, 300)  # In Folder
        self.files_table_widget.setColumnWidth(3, 100)  # Resolve Status
        self.files_table_widget.setColumnWidth(4, 80)  # Type
        self.files_table_widget.setColumnWidth(5, 100)  # Pending Action
        self.files_table_widget.setColumnWidth(6, 80)  # Changelist
        self.files_table_widget.setColumnWidth(7, 150)  # Entity Name
        self.files_table_widget.setColumnWidth(8, 80)  # Entity ID
        self.files_table_widget.setColumnWidth(9, 200)  # Context
        self.files_table_widget.setColumnWidth(10, 150)  # Comment

        # Enable sorting by clicking headers
        self.files_table_widget.setSortingEnabled(False)  # We'll handle sorting manually
        header.sectionClicked.connect(self.handle_header_click)

        # Enable context menu for the table
        self.files_table_widget.setContextMenuPolicy(Qt.CustomContextMenu)
        self.files_table_widget.customContextMenuRequested.connect(self.show_context_menu)
    ...
    def handle_header_click(self, logical_index):
        """
        Handle clicking on column headers to sort the table.
        """
    ...
    def sort_table(self, column, order):
        ...
        for row in range(self.files_table_widget.rowCount()):
            key = (self.files_table_widget.item(row, 1).text(),
                   self.files_table_widget.item(row, 2).text())
            checkbox_states[key] = self.files_table_widget.item(row, 0).checkState()

        # Enable sorting, sort, then disable again
        self.files_table_widget.setSortingEnabled(True)
        self.files_table_widget.sortItems(column, order)
        self.files_table_widget.setSortingEnabled(False)
        ...
        for row in range(self.files_table_widget.rowCount()):
            ...
            if key in checkbox_states:
                self.files_table_widget.item(row, 0).setCheckState(checkbox_states[key])

    def show_context_menu(self, position):
        """
        Show context menu for changing context of selected files.
        """
        # Get selected rows
        selected_rows = set()
        for item in self.files_table_widget.selectedItems():
        ...
        action = menu.exec_(self.files_table_widget.mapToGlobal(position))

        if action == change_context_action:
            self.change_context_for_selected_files(selected_rows)

    def change_context_for_selected_files(self, selected_rows):
        """
        Open a dialog to change the context for selected files.
        """
        # Try to use ShotGrid's context selector if available
        try:
            from sgtk.platform.qt import QtGui
            from tank.platform.qt.context_selector import GlobalSearchWidget

            # Create a dialog with context selector
            dialog = QDialog(self)
            dialog.setWindowTitle("Select Context")
            dialog.setMinimumWidth(600)
            dialog.setMinimumHeight(400)

            layout = QVBoxLayout()

            # Add context selector widget
            context_widget = GlobalSearchWidget(dialog)
            context_widget.set_searchable_entity_types(["Task", "Asset", "Shot", "Sequence", "Episode"])
            ...
            if dialog.exec_() == QDialog.Accepted:
                selected_entity = context_widget.get_selected_entity()
                if selected_entity:
                    self.apply_context_to_rows(selected_rows, selected_entity)

        except ImportError:
            # Fallback if GlobalSearchWidget is not available
            logger.warning("GlobalSearchWidget not available, using simple input dialog")
            context_str, ok = QInputDialog.getText(self, "Change Context",
                                                   "Enter context (e.g., Task:12345):")
            if ok and context_str:
                # Save the manual context
                for row in selected_rows:
                    self.apply_manual_context_to_row(row, context_str)

    def apply_context_to_rows(self, rows, entity):
        """
        Apply the selected entity context to the specified rows.
        """
        ...
        if not context_str:
            try:
                ...
                logger.error(f"Failed to get context for {entity_type} {entity_id}: {e}")
                return

        # Apply to rows and save
        for row in rows:
            file_key = (self.files_table_widget.item(row, 1).text(),
                        self.files_table_widget.item(row, 2).text())

            # Update table
            self.files_table_widget.setItem(row, 9, QTableWidgetItem(context_str))

            # Update entity info if available
            self.files_table_widget.setItem(row, 7, QTableWidgetItem(entity.get("name", "None")))
            self.files_table_widget.setItem(row, 8, QTableWidgetItem(str(entity_id)))

            # Save the context change
            self.saved_contexts[file_key] = {
                "context": context_str,
                "entity": entity
            }

            # Update the internal data structure
            if file_key in self.submit_widget_dict:
                sg_item = self.submit_widget_dict[file_key].get("sg_item", {})
                if sg_item:
    ...
    def apply_manual_context_to_row(self, row, context_str):
        """
        Apply a manually entered context string to a row.
        """
        file_key = (self.files_table_widget.item(row, 1).text(),
                    self.files_table_widget.item(row, 2).text())
        ...
        # Save the context
        self.saved_contexts[file_key] = {"context": context_str}
    ...
    def load_saved_contexts(self):
        """
        Load saved contexts from persistent storage.
        """
        try:
            app = sgtk.platform.current_bundle()
            cache_dir = app.cache_location
            contexts_file = os.path.join(cache_dir, "submit_contexts.json")
            ...
            logger.warning(f"Failed to load saved contexts: {e}")
    ...
    def save_contexts(self):
        ...
        try:
            ...
            data = {list(k): v for k, v in self.saved_contexts.items()}
    ...
    def set_cell_tooltip(self, row, col, text):
        """
        Set tooltip for a table cell with the full cell contents.
        """
    ...
    def populate_file_table(self):
        ...
        if self.submit_widget_dict:  # Only process if there are files
            entity_thread = threading.Thread(target=self.process_entities,
        ...
        for key, file_info in sorted_file_items:
            ...
            # Check for saved context override
            saved_info = self.saved_contexts.get(key, {})
            if saved_info:
                # Override with saved context
                context_str = saved_info.get("context", sg_item.get("context", "None") if sg_item else "None")
                if saved_info.get("entity"):
                    entity = saved_info["entity"]
            else:
            ...
            # Column 1: File
            file_text = file_info.get("file", "N/A")
            file_item = QTableWidgetItem(file_text)
            self.set_cell_tooltip(row_position, 1, file_text)
            self.files_table_widget.setItem(row_position, 1, file_item)

            # Column 2: In Folder
            folder_text = file_info.get("folder", "N/A")
            folder_item = QTableWidgetItem(folder_text)
            self.set_cell_tooltip(row_position, 2, folder_text)
            self.files_table_widget.setItem(row_position, 2, folder_item)

            # Column 3: Resolve Status
            resolve_text = file_info.get("resolve_status", "N/A")
            resolve_item = QTableWidgetItem(resolve_text)
            self.set_cell_tooltip(row_position, 3, resolve_text)
            self.files_table_widget.setItem(row_position, 3, resolve_item)

            # Column 4: Type
            type_text = file_info.get("type", "N/A")
            type_item = QTableWidgetItem(type_text)
            self.set_cell_tooltip(row_position, 4, type_text)
            self.files_table_widget.setItem(row_position, 4, type_item)

            # Column 5: Pending Action
            action_text = file_info.get("pending_action", "N/A")
            action_item = QTableWidgetItem(action_text)
            self.set_cell_tooltip(row_position, 5, action_text)
            self.files_table_widget.setItem(row_position, 5, action_item)
            ...
            elif sg_item and sg_item.get("change") and not sg_item.get(
                    "headChange"):  # Fallback if headChange is missing but change exists
                source_changelist_id_str = str(sg_item.get("change"))

            cl_item = QTableWidgetItem(source_changelist_id_str)
            self.set_cell_tooltip(row_position, 6, source_changelist_id_str)
            self.files_table_widget.setItem(row_position, 6, cl_item)
            ...
            if entity and isinstance(entity, dict):
                ...
                entity_name_item = QTableWidgetItem(str(entity_name))
                self.set_cell_tooltip(row_position, 7, str(entity_name))
                self.files_table_widget.setItem(row_position, 7, entity_name_item)

                entity_id_item = QTableWidgetItem(str(entity_id))
                self.set_cell_tooltip(row_position, 8, str(entity_id))
                self.files_table_widget.setItem(row_position, 8, entity_id_item)

                context_item = QTableWidgetItem(str(context_str))
                self.set_cell_tooltip(row_position, 9, str(context_str))
                self.files_table_widget.setItem(row_position, 9, context_item)

                comment_text = "Entity is recognizable"
                comment_item = QTableWidgetItem(comment_text)
                self.set_cell_tooltip(row_position, 10, comment_text)
                self.files_table_widget.setItem(row_position, 10, comment_item)
    ...
    def get_entity(self, sg_item):
        """
        Get the entity name and id from the Shotgun item, prioritizing the most specific context (Task).

        :param sg_item: Standard Shotgun entity dictionary with keys type, id and name.
        :return: Updated sg_item with entity and context information.
        """
        if sg_item:
            entity, published_file = self.parent.get_entity_from_sg_item(sg_item)

            # Try to get the most specific context (Task level if available)
            most_specific_entity = None
            context_str = None
            ...
            if entity:
                ...
                # First try to get context from the entity
                if entity_type and entity_id:
                    try:
                        tk = sgtk.sgtk_from_entity(entity_type, entity_id)
                        context = tk.context_from_entity(entity_type, entity_id)

                        # Check if context has a task - if so, use the task as the entity
                        if context.task:
                            most_specific_entity = context.task
                            entity_type = "Task"
                            entity_id = context.task["id"]
                        else:
                            most_specific_entity = entity
                ...
                # If we found a more specific entity, use it
                if most_specific_entity:
                    sg_item["entity"] = most_specific_entity
                else:
                    sg_item["entity"] = entity
            ...
            if file_name and folder:
                ...
                if key in self.submit_widget_dict:
                    ...
                    if full_file_info:
                        ...
                        # Apply any saved context overrides
                        if key in self.saved_contexts:
                            saved_info = self.saved_contexts[key]
                            if "context" in saved_info:
                                full_file_info["sg_item"]["context"] = saved_info["context"]
                            if "entity" in saved_info:
                                full_file_info["sg_item"]["entity"] = saved_info["entity"]
