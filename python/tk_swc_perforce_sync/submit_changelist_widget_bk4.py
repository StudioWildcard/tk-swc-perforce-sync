...
class SubmitChangelistWidget(QtGui.QDialog):
    ...
    def __init__(self, parent=None, myp4=None, change_item=None, file_dict=None):
        ...
        # Connect context widget change signal
        self.context_widget.context_changed.connect(self._on_context_widget_changed)
    ...
    def _setup_files_table(self):
        ...
        self.files_table_widget.itemSelectionChanged.connect(self._on_table_selection_changed)
    ...
    def _on_table_selection_changed(self):
        """Called when the table selection changes to update the context widget."""
        selected_rows = self._get_selected_rows()

        if not selected_rows:
            # No selection, clear context widget
            self.context_widget.enable_editing(
                False,
                "<p>Select one or more items to view/edit their context.</p>"
            )
            self.context_widget.set_context(None)
            return

        # Collect contexts from selected rows
        contexts = []
        ...
        for row in selected_rows:
            ...
            if dict_key and dict_key in self.submit_widget_dict:
                sg_item = self.submit_widget_dict[dict_key].get("sg_item", {})
                entity = sg_item.get("entity", {})

                # Check if entity is recognized
                if isinstance(entity, dict) and entity.get("type") and entity.get("id"):
                    # Try to get the context for this entity
                    context_str = sg_item.get("context", "")

                    # For recognized entities, we need to recreate the context object
                    try:
                        # Get task if available
                        task = sg_item.get("task")
                        if task and isinstance(task, dict) and task.get("type") == "Task" and task.get("id"):
                            # Use task for context
                            tk = sgtk.sgtk_from_entity("Task", task["id"])
                            context = tk.context_from_entity("Task", task["id"])
                        else:
                            # Use entity for context
                            tk = sgtk.sgtk_from_entity(entity["type"], entity["id"])
                            context = tk.context_from_entity(entity["type"], entity["id"])

                        contexts.append(context)
                    except Exception as e:
                        logger.debug(f"Could not create context for entity {entity}: {e}")
                        all_contexts_valid = False
                else:
                    # Entity not recognized
                    all_contexts_valid = False

        # Update context widget based on what we found
        if not all_contexts_valid:
            # Some entities are not recognized, disable editing
            self.context_widget.enable_editing(
                False,
                "<p>Some selected items have unrecognized entities. Context cannot be edited.</p>"
            )
            self.context_widget.set_context(None)
        elif not contexts:
            ...
            self.context_widget.enable_editing(
                False,
                "<p>No valid contexts found for selected items.</p>"
            )
            self.context_widget.set_context(None)
        elif len(contexts) == 1:
            # Single context - show it and allow editing
            self.context_widget.enable_editing(
                True,
                "<p>Task and Entity Link for selected item:</p>"
            )
    ...
    def _on_context_widget_changed(self, context):
        """Called when the context is changed in the context widget."""
        selected_rows = self._get_selected_rows()
        ...
        self._apply_new_context(entity_type, entity_id, entity_name, selected_rows, context)
    ...
    def populate_file_table(self):
        ...
        # Initially disable context editing until something is selected
        self.context_widget.enable_editing(
            False,
            "<p>Select one or more items to view/edit their context.</p>"
        )
    ...
    def get_entity(self, sg_item):
        ...
        if isinstance(task, dict) and task.get("type") == "Task" and task.get("id"):
            context_entity = task
            sg_item["task"] = task  # Store task in sg_item for later use
            ...
            if 'entity' in state or 'task' in state:
                try:
                    ...
                    if context:
                        ...
                        logger.debug(f"Restored context: {context}")
                except Exception as e:
                    logger.warning(f"Failed to restore context: {e}")

            logger.debug(f"Loaded saved state from {config_file}")
